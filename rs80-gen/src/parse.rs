      ...
      I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    ...
    let captured_bits = letter()
        .then(|c| {
            many(char(c)).map(move |s: Vec<_>| PatPart::Var(c, s.len() + 1))
        });
      ...
      I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    choice((
        ...
        char('#').with(choice((
            char('#').with(letter()).map(|c| Operand::I(c, IType::I16)),
            letter().map(|c| Operand::I(c, IType::I8)),
        ))),
        char('&').with(letter()).map(|c| Operand::F(c, FType::RP)),
        char('?').with(letter()).map(|c| Operand::F(c, FType::RM)),
        char('@').with(letter()).map(|c| Operand::I(c, IType::Address)),
        char('*').with(letter()).map(|c| Operand::F(c, FType::C3)),
    ))
}
